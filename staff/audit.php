...
// ===== FETCH STAFF AUDIT LOGS =====
$sql = "
    -- 1. Messages Sent by Staff
    SELECT 
    ...
    -- 2. Staff System Activities (Login, Logout, etc.)
    SELECT 
    ...
    WHERE u.user_id = $user_id
      AND a.activity NOT LIKE 'Staff sent mass notification%'

    UNION ALL

    -- 3. Staff Alerts Created
    SELECT 
        ...
        NULL AS target,
        NULL AS target_role,
        CONCAT('Alert Created: ', al.title, ' - ', al.message) AS details,
        al.created_at AS timestamp,
        'Staff Alert' AS log_type,
        NULL AS ip_address
    FROM alerts al
    JOIN users u ON al.created_by = u.user_id
    WHERE u.user_id = $user_id
   ...
   <!-- Sidebar -->
    <aside class="w-64 bg-gray-900 text-white flex flex-col overflow-y-auto h-screen">
        ...
        <nav class="flex-1 p-4">
            ...
            <a href="audit.php" class="block py-2 px-3 rounded bg-gray-700">Audit Logs</a>
        </nav>
